...
def get_link_subcategory(link_category):
    ...
    for soup_tag in soup_links_subcategory:
        ...
        try:
            ...
            description = ''
        dict_link_subcategory = {'link_subcategory': link_subcategory,
    ...
    return list_link_subcategories, pickle_file
...
def get_link_subject(link_subcategory):
    ...
    list_link_subject = []
    subject_tail = link_subcategory['link_subcategory']
...
def get_full_info_kabel(subject, link_subcategory):
    # Проходит по ссылкам и получает полную информацию о КАБЕЛЕ (товаре)
...
def get_output_merg():
    ...
    tail = KABEL                                                        # определяет начало url для парсинга (в данном случае силовой кабель)
    ...
    for link_category in list_link_category:                            # проходит по каждой ссылке списка
        list_link_subcategories = get_link_subcategory(link_category)   # получает список ссылок на странице 4 уровня
...
def get_output_category():
    ...
    # Пример - /catalog/kabel/rezinovoj/

    tail = KABEL
    url_category = '/catalog/kabel/rezinovoj/'
    content = fetch_url(tail)
...
def get_output_subcategory():
    ...
    # Пример - /kabel/rezinovoj/kg/

    tail = KABEL
    url_subcategory = '/catalog/kabel/rezinovoj/kg/'
    content = fetch_url(tail)
    soup_with_links = get_soup_links(content)
    list_link_categories = get_link_category(soup_with_links)
    for link_category in list_link_categories:
...
def get_output_subject():
    ...
    # Пример - /catalog/kabel/rezinovoj/kg/1-10/

    tail = KABEL
    url_subject = '/catalog/kabel/rezinovoj/kg/1-10/'
    content = fetch_url(tail)
    soup_with_links = get_soup_links(content)
    list_link_categories = get_link_category(soup_with_links)
    for link_category in list_link_categories:
        ...
        for link_subcategory in list_link_subcategories:                # ищет нужный кабель
            ...
            for subject in list_subjects:                               # проходит по этому списку
                full_info_subjects = get_full_info_kabel(
                    subject, link_subcategory)
                if subject['link_subject'] == url_subject:
                    get_full_info_kabel(subject, link_subcategory)
                    list_full_info_subjects.append(full_info_subjects)  # записывает результат парсинга в pickle файл
                else:
                    continue
